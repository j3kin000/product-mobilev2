...
export const CreateTask = () => {
  ...
  const [selectedGroupItem, setGroupSelectedItem] = useState(
    t('selectedGroup'),
  );
  ...
  useEffect(() => {
    ...
    const res = formToShow.reduce((acc, x) => {
      if (
        ['statusId', 'taskId', 'taskType', 'assignedTo', 'createdAt'].includes(
          x.key,
        )
      ) {
        return acc;
  ...
  const handleElementField = async (key, value) => {
    ...
    for (const field of formToShow) {
      ...
      if (field.key == 'appliancesList') {
        console.log('valid', valid);
      }
      if (!valid) {
        res[field.key] = Array.isArray(res[field.key]) ? [] : '';
      }
    }
    setValues(res);
  };
  ...
  useEffect(() => {
    const initialValues = formToShow.reduce((acc, item) => {
      ...
      if (skip.includes(item.inputType)) {
        return acc;
      }
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={{flexDirection: 'row'}}>
          ...
          <Text style={[styles.headerText, {marginLeft: scale(250)}]}>
            {timeNow}
          </Text>
        </View>
      </View>
        ...
        {components.length ? (
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              const createNewTaskResult = await createNewTask(
                values,
                typeSelectedKey,
                IdToken,
                tenant,
